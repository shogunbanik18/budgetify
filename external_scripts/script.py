...
if __name__ == "__main__":
    target_directory = '/opt/airflow/external_scripts'
    ...
    with open("/opt/airflow/external_scripts/config.json", "r") as config_file:
    ...
    root_directory = "/opt/airflow/data"
    ...
    try:
        ...
        #####
        # schema creation
        schema_name = f"finance_insights_l0"   
        ...
        try:
            ...
            conn.commit()
            util.log_info(f"Schema '{schema_name}' created successfully!")
            
            # Check if the schema exists (after creation or if it was already there)
            ...
            # If results are not None, the schema exists
            if results:
                util.log_info(f"Schema '{schema_name}' already exists.")
            else:
                util.log_info(f"Schema '{schema_name}' does not exist but was created.")

        except Exception as e:
            util.log_info(f"Error creating or checking schema '{schema_name}': {e}")
            raise


        # Iterating in the root director
        # data_folder = os.path.join(root_directory, "data")
        # util.log_info(f"data folder path is {data_folder} ")
        for file_name in os.listdir(root_directory):
            if file_name.endswith(".xlsx") and file_name == "Mock_data_fy_25_26.xlsx":
            # if file_name.endswith(".xlsx") and file_name == "FY_2025_26.xlsx":
                ...
                for sheets in sheet_names:
                    ...
                    # saving the df to a temporary csv file in root_directory
                    temp_csv_file = os.path.join(root_directory, f"{table_name}.csv")
