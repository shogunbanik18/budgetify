...
def load_data(current_schema, target_schema, table_name):
    ...
    try:
        ...
        util.log_info(source_table)
        util.log_info(target_table)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if target_schema == "finance_insights_l2":
            util.log_info("Storing the views in a temp layer ")
            cursor.execute("call finance_insights_l2.store_view_definitions();")
            util.log_info("Droping the views for proper data load to l2 ")
            cursor.execute("call finance_insights_l2.drop_all_views();")
        ...
        if target_schema == "finance_insights_l2":
            ...
            cursor.execute("call finance_insights_l2.recreate_all_views();")
            conn.commit()
